...
class Waveforms(QObject):
    ...
    def __init__(self,microcontroller):
        ...
        self.pressure_array = {}
        for i in range(NUMBER_OF_CHANNELS):
            self.pressure_array[str(i)] = deque(maxlen=self.maxLen)
    ...
    def update_waveforms(self):
        ...
        if readout is not None:
            ...
            for i in range(MCU.TIMEPOINT_PER_UPDATE):
                ...
                self.time = (self.time_ticks - self.time_ticks_start)*MCU.TIMER_PERIOD_ms/1000 # Time in seconds
                ...
                # channel readings
                for k in range(NUMBER_OF_CHANNELS):
                    # Holds the channel data for a specific data chunk
                ...
                # Write the data to a csv file if logging is ON

                # line to write
                record_from_MCU = str(self.time) + ','
                ...
                #-----------
                # We have read and converted data from the uController now...
                # I want to modify the way stuff is done after this
                #--------
            
            # Assemble the data chunks into a Deque (so we don't have arrays that keep increasing in size over time!)
            ...
            if IS_PRESSURE_MEASUREMENT:
                for k in range(NUMBER_OF_CHANNELS):
                    ...
                    self.pressure_array[str(k)].append([pressure_chunck[str(k)]])
            ...
            if self.counter_display >= WAVEFORMS.DISPLAY_UNDERSAMPLING:
                ...
                self.counter_display = 0
                # emit plots
                plot_arrays = pressure_chunck[str(0)]

                for k in range(1,NUMBER_OF_CHANNELS_DISPLAY):

                    plot_arrays = np.vstack((plot_arrays, pressure_chunck[str(k)]))
